            ...
            <div className={`flex flex-col justify-start items-start bg-inherit  font-normal space-x-3 text:sm rounded-full rounded-l-none ${pathName === `/schoolmgt/subject/deeje/subjectfield` && ' rounded-full rounded-l-none '}`}>
                ...
                <div className="items-start list-none w-[278px] bg-none">
                    {tab3 && dataset.map((data) =>(
                        <Link key={data._id} href={`schoolmgt/subject/${data._id}`} className={`flex font-normal space-x-3 text:sm py-3 items-center hover:bg-secondary-200 rounded-full rounded-l-none ${pathName === `schoolmgt/subject/${data._id}`&& 'bg-purple-600 rounded-full rounded-l-none border text-white '}`}>class: {data.name}</Link>
                    ))}
                </div>
            </div>
        </div>
