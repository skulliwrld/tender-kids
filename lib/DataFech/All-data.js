...
// Cached version of teacher fetching with pagination
export const fetchTeachers = cache(async (q, page = 1) => {
...
// Cached version - avoids repeated DB calls for same data
export const AllTeachersFetch = cache(async () => {
    try {
        await connectToDB();
...
export const AllClassFetch = cache(async () => {
    try {
        await connectToDB();
        const classess = await Class.find()
        if (!classess) {
            console.log("there was an issue when getting classes data")
        }
...
export const AllSubjectFetch = cache(async () => {
    try {
        await connectToDB();
        const Subjects = await Subject.find()
        if (!Subjects) {
            console.log("there was an issue when getting subjects data")
        }
...
export const FetchClasses = cache(async (q, teacherId = null) => {
    try {
        await connectToDB();
...
export const fetchIndividualClassData = cache(async (id) => {
    try {
        await connectToDB()
...
export const AllDayFetch = cache(async () => {
    try {
        await connectToDB();
        const days = await Day.find()
        if (!days) {
            console.log("there was an issue when getting days data")
        }
...
export const AllStudentFetch = cache(async () => {
    try {
        await connectToDB();
...
export const fetchStudentsByClass = cache(async (classId) => {
    try {
        await connectToDB();
...
export const fetchIndividualStudentData = cache(async (id) => {
    try {
        await connectToDB()
