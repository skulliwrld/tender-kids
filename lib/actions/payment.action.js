...
function getEffectiveStudentFeeAmount(studentFee) {
  const baseAmount = Number(studentFee?.amount || 0);
  const arrearsAmount = Number(studentFee?.arrears || 0);
  const waiverAmount = studentFee?.isWaived ? Number(studentFee?.waiverAmount || 0) : 0;

  return Math.max(0, baseAmount + arrearsAmount - waiverAmount);
}
...
export async function createPayment(data) {
  ...
  try {
    ...
    const finalAmount = getEffectiveStudentFeeAmount(studentFee);
...
export async function getPayments(filters = {}) {
  ...
  // Filter out payments where the referenced student no longer exists
  return payments.filter(payment => payment.student !== null);
}
...
export async function getStudentPayments(studentId, academicSessionId = null, termId = null, paymentDate = null) {
  ...
  const payments = await Payment.find(query)
    ...
    .populate('studentFee', 'feeStructure amount feeCategory title')
    .populate('studentFee.feeStructure', 'name')
    .populate('academicSession', 'name')
    .populate('term', 'name termNumber')
    .sort({ paymentDate: -1 });
  
  // Filter out payments where the referenced student or studentFee no longer exists
  return payments.filter(payment => payment.student !== null && payment.studentFee !== null);
}
...
export async function getPaymentHistory(studentFeeId) {
  await connectToDB();
  const payments = await Payment.find({ studentFee: studentFeeId, status: 'completed', isDeleted: false })
    .populate('studentFee', 'feeStructure amount feeCategory title')
    .populate('studentFee.feeStructure', 'name')
    .sort({ paymentDate: -1 });
  
  // Filter out payments where the referenced studentFee no longer exists
  return payments.filter(payment => payment.studentFee !== null);
}
