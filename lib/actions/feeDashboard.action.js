...
const totalArrearsExpression = {
  $cond: [
    { $eq: ['$feeCategory', 'arrears'] },
    '$balance',
    { $ifNull: ['$arrears', 0] },
  ],
};

export async function getAdminFeeDashboard(academicSessionId = null, termId = null, paymentDate = null) {
  await connectToDB();
  ...
  const paymentDateMatch = paymentDate ? { paymentDate: { $gte: new Date(paymentDate), $lt: new Date(new Date(paymentDate).getTime() + 24 * 60 * 60 * 1000) } } : {};
  ...
  const overallSummary = await StudentFee.aggregate([
    { $match: {} },
    {
      $group: {
        _id: null,
        totalExpected: { $sum: { $add: ['$amount', { $ifNull: ['$arrears', 0] }] } },
        totalPaid: { $sum: '$amountPaid' },
        totalBalance: { $sum: '$balance' },
        totalArrears: { $sum: totalArrearsExpression },
        feesCount: { $sum: 1 },
      },
    },
  ]);
  ...
  const statusBreakdown = await StudentFee.aggregate([
    { $match: matchStage },
    {
      $group: {
        _id: '$status',
        count: { $sum: 1 },
        amount: { $sum: { $add: ['$amount', { $ifNull: ['$arrears', 0] }] } },
  ...
  const collectionByMethod = await Payment.aggregate([
    { 
      $match: { ...matchStage, status: 'completed', isDeleted: false } 
    },
    {
      $lookup: {
        from: 'students',
  ...
  const recentPayments = (await Payment.find({ ...matchStage, ...paymentDateMatch, status: 'completed', isDeleted: false })
    .populate('student', 'Name')
    .populate('studentFee')
    .sort({ paymentDate: -1 })
    .limit(paymentDate ? 100 : 10))
    .filter(payment => payment.student && payment.studentFee);
  ...
  return {
    overallSummary: overallSummary[0] || { totalExpected: 0, totalPaid: 0, totalBalance: 0, totalArrears: 0, feesCount: 0 },
    periodSummary: periodSummary[0] || { totalExpected: 0, totalPaid: 0, totalBalance: 0, totalArrears: 0, feesCount: 0 },
    statusBreakdown,
    collectionByMethod,
    recentPayments,
    topDebtors,
...
export async function getClassFeeReport(classId, academicSessionId, termId = null) {
  ...
  return {
    ...
    totals: totals[0]
      ? { ...totals[0], studentCount: totals[0].studentCount?.length || 0 }
      : { totalExpected: 0, totalPaid: 0, totalBalance: 0, totalArrears: 0, studentCount: 0 },
  };
}
