...
export async function createStandaloneArrear(data) {
  await connectToDB();

  if (!data.student || !data.academicSession || !data.term) {
    throw new Error('Student, academic session and term are required');
  }

  const amount = Number(data.amount || 0);
  if (amount <= 0) {
    throw new Error('Arrears amount must be greater than 0');
  }

  const arrear = await StudentFee.create({
    student: data.student,
    academicSession: data.academicSession,
    term: data.term,
    feeCategory: 'arrears',
    title: data.title?.trim() || 'Student Arrears',
  ...
  return await getStudentFeeById(arrear._id);
}
...
export async function getStudentFeeSummary(studentId, academicSessionId = null, termId = null) {
  ...
  const result = await StudentFee.aggregate([
    { $match: matchStage },
    {
      $group: {
        ...
        totalAmount: { $sum: { $add: ['$amount', { $ifNull: ['$arrears', 0] }] } },
  ...
  return {
    summary: result[0] || { totalOriginalAmount: 0, totalAmount: 0, totalPaid: 0, totalBalance: 0, totalArrears: 0, totalWaived: 0, feesCount: 0 },
    statusBreakdown,
  };
}
